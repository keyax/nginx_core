...
function listings(){
 jQuery.ajax({
    url: "http://kyx.dynu.net/pets/listad",
    type: "GET",
    dataType: "jsonp"//,
...
.then ((response) => {
 //  if(response) {return JSON.stringify(response.json());}  //return response.body.json();
     if(response) {return response;}  //return response.body.json();
...
.then((response) => {
  alert(response.text());
  document.getElementById("langs").innerHTML = response;

})
.catch(function(error) {
    // This is where you run code if the server returns any errors
    console.log("ERREUR:"+ error);//JSON.stringify(error)
});
//await next();
...
function processData(data) {
    ...
    console.log("datalog" + datax );

    document.getElementById("langs").innerHTML = datax;
}
